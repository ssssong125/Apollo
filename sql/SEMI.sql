...
COMMENT ON COLUMN TBL_USER.USER_TEL IS '전화번호';
COMMENT ON COLUMN TBL_USER.ADDRESS IS '주소';
COMMENT ON COLUMN TBL_USER.USER_EMAIL IS '이메일';
COMMENT ON COLUMN TBL_USER.USER_ROLE IS '활성화상태';
COMMENT ON COLUMN TBL_USER.USER_ACTIVE IS '유저권한';
COMMENT ON COLUMN TBL_USER.USER_ENTDATE IS '가입날짜';
COMMENT ON COLUMN TBL_USER.USER_POINT IS '포인트';
...
ALTER TABLE TBL_CART ADD CONSTRAINT FK_TBL_CART_PRODUCT_NO FOREIGN KEY (PRODUCT_NO) REFERENCES "TBL_PRODUCT"(PRODUCT_NO) ON DELETE CASCADE;
ALTER TABLE TBL_CART ADD CONSTRAINT FK_TBL_CART_USER_ID FOREIGN KEY (USER_ID) REFERENCES "TBL_USER"(USER_ID);
ALTER TABLE TBL_CART ADD CONSTRAINT CK_TBL_CART_CHECK_STATUS CHECK(CHECK_STATUS IN ('Y' ||
                                                                                    '', 'N'));
