...
DROP TABLE TBL_PURCHASED_CART CASCADE CONSTRAINTS;
...
-- 구매 완료 장바구니 테이블
CREATE TABLE TBL_PURCHASED_CART (
    CART_NO NUMBER NOT NULL,
    PRODUCT_NO NUMBER NOT NULL,
    USER_ID	VARCHAR(50) NOT NULL,
    PRODUCT_COUNT NUMBER NOT NULL,
...
ALTER TABLE TBL_PURCHASED_CART ADD CONSTRAINT PK_CART_NO_2 PRIMARY KEY (CART_NO);
...
BEGIN
    UPDATE TBL_USER A
    SET A.USER_POINT = A.USER_POINT - (SELECT
                                            ROUND(B.USED_POINT + (0.05 * B.PAYMENT_AMOUNT))
                                       FROM TBL_PAYMENT B
...
/
/*상품 재고 트리거*/
CREATE OR REPLACE TRIGGER TRG_PRODUCT_QTY
    AFTER INSERT ON TBL_PURCHASED_CART
    FOR EACH ROW
BEGIN
    UPDATE TBL_PRODUCT A
    SET A.PRODUCT_QTY = A.PRODUCT_QTY - (SELECT
                                            B.PRODUCT_COUNT
                                         FROM TBL_PURCHASED_CART B
                                         WHERE A.PRODUCT_NO = B.PRODUCT_NO)
    WHERE A.PRODUCT_NO = :NEW.PRODUCT_NO;
END;
/
/*결제시 장바구니테이블에서 결제완료테이블로 이동*/
-- CREATE OR REPLACE TRIGGER TRG_PURCHASED
--     BEFORE DELETE ON TBL_CART
--     FOR EACH ROW
-- BEGIN
--     IF :OLD.PURCHASE_STATUS = 'Y'
--     THEN
--     INSERT INTO TBL_PURCHASED_CART
--         VALUES (:OLD.CART_NO, :OLD.PRODUCT_NO, :OLD.USER_ID, :OLD.PRODUCT_COUNT, :OLD.REG_DATE, :OLD.CHECK_STATUS, :OLD.PURCHASE_STATUS);
--     END IF;
-- END;
-- /
/*결제시 구매상태 y로 바꾸면 자동으로 이동 장바구니테이블에서 결제완료테이블로 이동*/
CREATE OR REPLACE TRIGGER TRG_PURCHASED
    AFTER UPDATE ON TBL_CART
    FOR EACH ROW
BEGIN
    IF :NEW.PURCHASE_STATUS = 'Y'
    THEN
        INSERT INTO TBL_PURCHASED_CART
        VALUES (:OLD.CART_NO, :OLD.PRODUCT_NO, :OLD.USER_ID, :OLD.PRODUCT_COUNT, :OLD.REG_DATE, :OLD.CHECK_STATUS, :OLD.PURCHASE_STATUS);

        DELETE FROM TBL_CART
        WHERE CART_NO = :OLD.CART_NO;
    END IF;
END;
/
/*결제시 오더테이블에 추가*/
...
--         VALUES(SEQ_ORDER_NO.NEXTVAL, SEQ_PAYMENT_NO.CURRVAL, 카트 넘버);
--
-- END;
-- /
